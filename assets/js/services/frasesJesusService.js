...
const frasesJesusService = {
  async obterFraseAtual(forcar = false) {
    if (!forcar) {
    ...
    if (frase) {
      storageCore.set(FRASE_JESUS_KEY, { frase, timestamp: Date.now() });
    }
  ...
  async _coletarCandidatas() {
    ...
    for (const livro of livros) {
      ...
      seccoes.forEach((seccao, seccaoIndex) => {
        ...
        const paragrafoIndex = this._encontrarParagrafo(
          seccao.texto_sagrado?.paragrafos,
          texto,
        );
  ...
  _encontrarParagrafo(paragrafos, textoAlvo) {
    ...
    const index = paragrafos.findIndex((p) =>
      this._limparHTML(p).includes(textoAlvo),
    );
