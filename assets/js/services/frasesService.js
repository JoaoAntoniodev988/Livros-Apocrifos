...
const frasesService = {
  async obterFraseAtual(forcar = false) {
    if (!forcar) {
      ...
      if (
        cache &&
        agora - cache.timestamp < appConfig.fraseMomentoIntervaloMs
      ) {
        return cache.frase;
      }
    }
    ...
    if (frase) {
      storageCore.set(FRASE_MOMENTO_KEY, { frase, timestamp: Date.now() });
    }
  ...
  async _sortear() {
    ...
    const origem = usaParagrafos
      ? paragrafos
      : (citacoes?.length &&
          (Array.isArray(citacoes) ? citacoes : [citacoes])) ||
        null;
